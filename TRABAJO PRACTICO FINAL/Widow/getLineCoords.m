function [pos1, pos2,sum_bordes,warpedth_g,warpedth_r,final_linea]=getLineCoords(green_filter,green_filter_l,green_filter_l2,green_filter_l3,green_filter_l4,red_filter_l,debug_state,umax,vmax)
...
%     verde, retorna las dos coordenadas con NaN.
    ...
    if(isnan(pos1))
        ...
        [pos1,pos2,sum_bordes,warpedth_g,warpedth_r,final_linea] = getBorders(green_filter_l2, red_filter_l, umax, vmax, debug_state);
        if(isnan(pos1))
            % Intento 3: Filtrado solo
            [pos1,pos2,sum_bordes,warpedth_g,warpedth_r,final_linea] = getBorders(green_filter, red_filter_l, umax, vmax, debug_state);
            if(isnan(pos1))
                % Intento 4: Filtrado + limpiado x3
                [pos1,pos2,sum_bordes,warpedth_g,warpedth_r,final_linea] = getBorders(green_filter_l3, red_filter_l, umax, vmax, debug_state);
                if(isnan(pos1))
                    % Intento 5: Filtrado + limpiado grande
                    [pos1,pos2,sum_bordes,warpedth_g,warpedth_r,final_linea] = getBorders(green_filter_l4, red_filter_l, umax, vmax, debug_state);
                end
            end
        end
    end
